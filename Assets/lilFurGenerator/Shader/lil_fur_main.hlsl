...
float4 frag(v2f input) : SV_Target
{
    ...
    // Alpha
        ...
        clip(col.a - _Cutoff);
    #else
        col.a = input.uv4.y > -0.5 ? col.a * saturate(LIL_SAMPLE_2D(_FurMap, sampler_linear_repeatU_clampV, input.uv4).a + fwidth(input.uv4.y) * 2.0) : col.a;
    ...
    float3 albedo = col.rgb;
        ...
        col.rgb = clamp(col.rgb, albedo * _LightMinLimit, albedo);
    #else
        col.rgb *= lightColor;
    #endif
